# Hosté — požadavky a info

> Centrální přehled toho, co je potřeba zohlednit na straně hostů.
> **U každého požadavku uveď, od koho je** (jméno hosta / skupiny), ať víš, komu se ozvat. Pokud jméno neznáš: „kdo: neuvedeno — zjistit".
> Surové dotazy/zprávy od hostů ukládej do `RAW/Hoste/`.

## Podle hostů

### <Jméno hosta / skupiny>
- **Téma:** (ubytování / dieta / doprava / mazlíček / jiné)
- **Co řeší:**
- **Co je od nás:**
- **Stav:**

## Sdílená / obecná info

### Dietní omezení a alergie
> Posbírej od všech (formulář) a rozepiš jmenovitě. Promítni do oběda, večeře, rautu, snídaní i dortu.

| Host | Omezení | Promítnuto kam | Stav |
|---|---|---|---|
|  |  |  |  |

### Ubytování
- Kapacita (lůžka / přistýlky / stan):
- Roztřídění do pokojů (kdo s kým, kdo nechce sdílet):

### Mazlíčci
- Pravidlo místa:
- Koho se týká a jak vyřešeno:

### Doprava / spolujízda
-

### Instrukce pro hosty (k rozeslání)
- [ ] Parkování + plánek
- [ ] Přezouvání
- [ ] Deky na sezení venku
- [ ] Info o jídle / dietách
- [ ] Sdílené fotoalbum (QR)
- [ ] Koordinační skupina (WhatsApp)
